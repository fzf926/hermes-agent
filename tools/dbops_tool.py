...
def _build_dbops_schema_overrides() -> dict[str, Any]:
    ...
    return {
        "description": (
            ...
            "This tool is part of the 运营/老师问数 workflow: first clarify business "
            "scope, time range, statistical definition, and filters, then generate a "
            "safe SQL query. 缺少时间范围、业务对象、统计口径或筛选条件时先追问；"
            "多个相似候选表同时匹配时，先让用户确认目标表，并说明候选表差异；"
            "do not guess a risky full-scan query. "
            ...
            "audited SQL only and does not access the online database. 执行开关关闭时，"
            "只展示已审核 SQL，不要声称已经查询，不要编造结果。 "
